...
public class FURenderer implements IFURenderer, IModuleManager {
     ...
     * @param width      宽
     * @param height     高
     * @param cameraType 相机方向
     */
    public void rotateImage(byte[] img, int width, int height, int cameraType) {
        boolean isFrontCam = cameraType == Camera.CameraInfo.CAMERA_FACING_FRONT;
        ...
        byte[] output = mRotatedImage.mData;
        System.arraycopy(output, 0, img, 0, output.length);
    }
